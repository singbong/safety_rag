"""
semantic_split.py - PDF Semantic Chunking with Google Gemini Embeddings

주요 기능:
1. PDF 문서를 의미적 경계에서 청크로 분할
2. Google Gemini 임베딩 모델의 토큰 제한(2048) 준수
...
클래스 구조:
- GeminiEmbeddings: Google Gemini API 래퍼, 토큰 제한 처리
- PDFSemanticChunker: PDF 의미적 청킹 메인 클래스

함수별 역할:
[GeminiEmbeddings]
- __init__: API 키 설정 및 클라이언트 초기화
...
class GeminiEmbeddings(Embeddings):
    """
    Google Gemini API를 이용한 LangChain Embeddings 래퍼 클래스
    """
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-embedding-001"):
        ...
        if api_key is None:
            ...
            env_path = os.path.join(current_dir, "google.env")
