...
1. **제목:**
   - '`{place_name}` 행사 방문객을 위한 안전 안내'와 같이 행사명을 포함하여 명확하게 작성한다.

2. **행사 개요 및 핵심 안전 수칙:**
   - 행사명, 기간, 장소, 유형 등 기본 정보를 간단히 소개한다.
   - 방문객이 반드시 알아야 할 핵심 안전 수칙 2-3가지를 강조한다.

3. **상세 안전 가이드:**
   A. **공통 안전 수칙:**
      - 질서 유지, 개인 소지품 관리, 금연 구역 준수 등
   B. **행사 유형별 안전 수칙:**
      - {type}과 {category}의 특성을 고려한 맞춤 안전 수칙
   C. **장소/시설 관련 안전 수칙:**
      - 위험 구역, 전기/가스 시설, 안내데스크 등
   D. **행사 기간 관련 안전 수칙:**
      - {period} 시기의 날씨와 환경 특성을 고려한 주의사항

4. **비상시 행동 요령:**
   - 화재, 응급환자, 인파 밀집, 시설물 붕괴 등 상황별 대처법
   - 비상 연락처: {emergency_contact_name}({emergency_contact_phone})
   - 112, 119 등 공공 안전기관 연락처

5. **특별 고려 대상 안내:**
   - 어린이, 노약자, 장애인 등을 위한 안내사항

[중요 규칙]
- **정확성:** 모든 정보는 제공된 문서에 근거하여 작성한다.
- **명확성:** 누구나 이해하기 쉬운 표현을 사용한다.
- **실용성:** 구체적이고 실천 가능한 행동 지침을 제공한다.
- **신뢰성:** 공식 안내문으로서의 신뢰도를 유지한다.
- **구조화:** 제목, 소제목, 글머리 기호 등으로 가독성을 높인다.
