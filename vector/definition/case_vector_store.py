...
class VertexEmbeddings:
    ...
    def __init__(self, project_id: Optional[str] = None, region: str = "us-central1", model: str = "gemini-embedding-001"):
        ...
        self.region = region
        self.output_dir = "../data/case_data/embedding_data/"
...
class store_vector_db:
    ...
    def _load_all_pkl_files(self, pkl_directory: str):
        pkl_files = glob.glob(pkl_directory)
        all_documents = []
    ...
    def save_vector_store(self, save_path: str, file_name: str):
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        faiss.write_index(self.index, f"{save_path}/{file_name}.index")
    ...
    def create_vector_store(self, pkl_directory: str, file_name: str, save_path: Optional[str] = "../data/case_data/embedding_data/"):
        ...
        for i, doc in enumerate(documents):
            try:
                embedding = self.embedding_model.embedding_document(doc['content'])
        ...
        if save_path:
            self.save_vector_store(save_path, file_name)
    ...
    def load_vector_store(self, load_path: str):
        # .index 파일에서 FAISS 인덱스 로드
        index_path = load_path if load_path.endswith('.index') else f"{load_path}.index"
        self.index = faiss.read_index(index_path)
        
        # .pkl 파일에서 문서 데이터 로드
        pkl_path = load_path.replace('.index', '.pkl') if load_path.endswith('.index') else f"{load_path}.pkl"
        with open(pkl_path, 'rb') as f:
            data = pickle.load(f)
    ...
    def document_rerank(self, query: str, documents: List[Dict[str, Any]], k: int = 5):
        """
