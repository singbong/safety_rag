...
이 시스템은 Google Geocoding API와 Open-Meteo 날씨 API를 활용하여 행사 장소의 실시간 날씨 정보를 분석하고, 이를 기반으로 날씨 관련 안전 위험 요소를 예측하는 고급 기능을 제공합니다.

### 날씨 데이터 수집 및 분석 파이프라인

#### 1. **지리 좌표 변환 (Geocoding)**
- **Google Geocoding API**를 사용하여 행사 장소명(예: "서울, 여의도 한강공원")을 정확한 위도/경도 좌표로 변환합니다.
- 한국어 주소 지원으로 국내 행사 장소에 최적화되어 있습니다.

#### 2. **24시간 날씨 예보 수집**
- **Open-Meteo API**를 통해 변환된 좌표 기반으로 24시간 후까지의 상세 날씨 정보를 수집합니다.
- 수집되는 날씨 데이터:
  - **기온**: 현재 기온 및 체감온도 (°C)
  - **강수량**: 시간당 강수량 (mm) 및 강수 확률 (%)
  - **풍속**: 10m 높이에서의 풍속 (km/h)
  - **습도**: 상대습도 (%)
  - **구름량**: 구름 커버리지 (%)
  - **UV 지수**: 자외선 지수
  - **시정**: 가시거리 관련 정보

#### 3. **LLM 기반 날씨 분석 및 위험 요소 추출**
- **Gemini 2.5 Flash Lite** 모델을 사용하여 수집된 날씨 데이터를 전문적으로 분석합니다.
- 분석 기준:
  - **기온 및 체감온도**: 평년 기온 대비 편차, 온열질환/한랭질환 위험도 평가
  - **강수량**: 시간당 강수량에 따른 위험도 분류 (3mm 이하: 약한 비, 3-15mm: 보통 비, 15-30mm: 강한 비, 30mm 이상: 매우 강한 비)
  - **풍속**: 평균 풍속 대비 세기, 강풍으로 인한 시설물 파손 위험 평가
  - **UV 지수**: 기상청 기준 단계별 위험도 평가 (0-2: 낮음, 3-5: 보통, 6-7: 높음, 8-10: 매우 높음, 11+: 위험)
  - **습도**: 쾌적 습도 범위(40-60%) 대비 평가, 온열질환 위험도 분석
  - **구름/시정**: 시정 악화로 인한 교통사고 위험 분석

#### 4. **날씨 기반 검색어 생성**
분석된 날씨 정보를 바탕으로 다음과 같은 패턴으로 검색어를 자동 생성합니다:
- **기온 관련**: "폭염 온열질환자 발생 사례", "한파 동상 사고 예방"
- **강수 관련**: "시간당 20mm 강수량 도로 통제 사례", "빗길 교통사고 시야 미확보"
- **풍속 관련**: "강풍 간판 및 시설물 낙하 사고", "바람으로 인한 행사장 시설물 안전사고"
- **복합 요인**: "여름철 국지성 호우 침수 피해", "고온다습한 날씨 식중독 사고"

#### 5. **날씨 관련 사고 사례 검색**
- 생성된 검색어를 사용하여 벡터 스토어에서 유사한 기상 조건에서 발생했던 과거 사고 사례를 검색합니다.
- 검색된 사례는 안전 안내문의 "날씨 관련 주의사항" 섹션에 포함되어 방문객에게 구체적인 대비 방안을 제시합니다.

### 환경 변수 설정

날씨 API 기능을 사용하기 위해 다음 환경 변수를 설정해야 합니다:
...
#### 날씨 데이터 처리 (`get_weather.py`)
```python
def get_weather_forecast(address, api_key):
    # 1. Google Geocoding API로 주소를 좌표로 변환
    # 2. Open-Meteo API로 24시간 날씨 예보 수집
    # 3. 시간별 상세 날씨 정보를 구조화된 텍스트로 변환
```

#### 날씨 분석 및 검색어 생성 (`custom.py`)
```python
weather_query_generator_system = """
[역할 및 목표]
너는 기상 관련 사고 사례 분석 전문가로서, 주어진 기상 예보를 분석하여 
유사한 기상 조건에서 발생했던 과거 사고 사례를 검색하기 위한 최적의 검색어를 생성하는 AI다.
...
"""
```
...
1. **여름철 야외 행사**: 폭염, 국지성 호우, 높은 UV 지수 등에 대한 사전 경고 및 대비 방안 제공
2. **겨울철 실외 행사**: 한파, 강설, 빙판길 등에 대한 안전 수칙 안내
3. **봄/가을 행사**: 일교차, 강풍, 미세먼지 등 계절별 특수 기상 조건에 대한 주의사항 제공
4. **실시간 기상 변화 대응**: 행사 당일 급변하는 날씨에 대한 즉각적인 안전 정보 업데이트
