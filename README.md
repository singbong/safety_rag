...
2.  **재순위 (Reranking):
    -   초기 검색된 150개의 문서를 `VertexAIRank` 모델을 사용하여 질문과의 관련성이 높은 순으로 재정렬합니다.
        ...
        -   VertexAIRank에서 사용하는 임베딩 및 랭킹 모델은 `semantic-ranker-512@latest`입니다. 이 모델은 쿼리와 문서 모두에 대해 512차원 임베딩을 생성하고, 최신 LLM 기반의 의미적 비교를 통해 쿼리-문서 쌍의 관련성을 평가합니다. (공식 문서 및 Vertex AI 콘솔 기준)
