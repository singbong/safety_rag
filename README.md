...
행사 및 재난 안전 관련 문서를 기반으로 사용자의 질문에 정확한 답변을 제공하는 RAG(Retrieval-Augmented Generation) 시스템입니다.

Google Gemini 모델과 LangChain/LangGraph 프레임워크를 활용하여 안전 지식의 검색 증강 생성을 구현했습니다.
...
### 📄 문서 처리
- **PDF 텍스트 추출**: `pdfplumber`를 사용하여 문서 내용 추출
- **OCR 처리**: Google Cloud Vision API를 사용하여 이미지 기반 PDF 텍스트 추출
- **의미 기반 분할(Semantic Chunking)**: 문맥을 고려한 지능형 문서 분할
- **컨텍스트 보강**: Anthropic의 Contextual Retrieval 기법 적용

### 🔍 지능형 검색
- **하이브리드 검색**: 의미 검색(70%) + 키워드 검색(30%) 결합
- **재순위(Re-ranking)**: Vertex AI Reranker를 통한 검색 결과 정밀화
- **다중 벡터 스토어**: 안전 문서, 날씨 사고 사례, 행사 사고 사례 별도 관리

### 🤖 질의응답 체인
- **일반 채팅**: 안전 지식에 대한 대화형 질의응답
- **양식 기반 안전 안내문**: 행사 정보를 입력하면 맞춤형 안전 가이드 생성
- **날씨 기반 분석**: 실시간 날씨 정보를 활용한 맞춤형 안전 안내문 생성
- **후속 질문 처리**: 생성된 안내문에 대한 추가 질문 지원
...
**필수 데이터**: 이 프로젝트를 실행하려면 다음 Hugging Face 데이터셋에서 데이터를 다운로드해야 합니다.

```
https://huggingface.co/datasets/bong9513/safety_rag_data
```

**관련 프로젝트**: 본 프로젝트는 [safety-navigator](https://github.com/sumkyun/safety-navigator)와 연동됩니다.
...
| **검색** | FAISS, BM25Retriever, Vertex AIRank |
| **데이터 처리** | pdfplumber, pandas, numpy |
| **배포** | Docker, Docker Compose |
| **외부 API** | Google Cloud Vision, Google Geocoding, Open-Meteo Weather |
...
├── fire_app.py                # FastAPI 메인 서버
├── requirements.txt           # Python 패키지 의존성
├── README.md                  # 이 문서
└── vector/                    # 데이터 및 정의 폴더
    ├── data/                  # 데이터 저장소
    │   ├── case_data/         # 사고 사례 데이터
    │   │   ├── original_data/ # 원본 사고 사례
    │   │   │   ├── case_climate.pkl      # 날씨 관련 사고 사례
    │   │   │   └── case_festival.pkl     # 행사 관련 사고 사례
    │   │   └── contextual_content_docs/  # 컨텍스트 보강된 문서
    │   ├── chunked_docs/       # 의미 기반으로 분할된 문서
    │   ├── original_full_text/ # PDF 원본 텍스트
    │   ├── original_pdf/       # 분석할 PDF 원본
    │   └── vector_store/       # FAISS 벡터 DB
    └── definition/             # RAG 파이프라인 정의
        ├── chain_for_chat.py        # 일반 채팅 체인
        ├── chain_for_form.py        # 양식 기반 안전 안내문 체인
        ├── chain_for_form_chat.py   # 후속 질문 처리 체인
        ├── custom.py                # 날씨 기반 안전 분석 체인
        ├── get_weather.py           # 날씨 데이터 처리
        ├── ocr.py                   # OCR 처리
        ├── make_context.py          # 컨텍스트 생성
        ├── make_contextual_content_with_caching.py  # 캐싱 적용 컨텍스트 생성
        ├── make_docs.py             # 문서 분할
        ├── make_full_text.py        # 텍스트 추출
        ├── make_vector_store.py     # 벡터 스토어 생성
        ├── vector_store.py          # 벡터 DB 관리
        ├── case_vector_store.py     # 사고 사례 벡터 스토어
        ├── semantic_split_genai.py  # GenAI 의미 분할
        └── semantic_split_vertex.py # Vertex AI 의미 분할
```
...
서비스 계정 인증 키를 `vector/definition/` 디렉토리에 배치하세요.
...
Hugging Face에서 데이터셋을 다운로드하여 `vector/data/` 디렉토리에 배치하세요.
...
### 2. 양식 기반 안전 안내문 생성 (`/api/generate_form`)

행사 정보를 입력하면 맞춤형 안전 안내문을 생성합니다.
...
response = requests.post("http://localhost:8000/api/generate_form", json={
    ...
    "description": "다양한 장르의 아티스트들과 함께하는 3일간의 축제",
    "category": "음악/페스티벌",
    "related_documents": "festival_guide.pdf",  # 행사 관련 문서 (OCR 처리됨)
...
### 3. 후속 질문 (`/api/form_chat`)

생성된 안내문에 대해 추가 질문을 합니다.

```python
# 먼저 /api/generate_form으로 생성된 안내문 사용
generated_form = "..."  # 이전 API 응답의 final_answer
...
### 4. 날씨 기반 맞춤형 안전 안내문 (`/api/custom_form`)

실시간 날씨 정보를 활용한 고급 안전 분석을 제공합니다.
...
response = requests.post("http://localhost:8000/api/custom_form", json={
    ...
    "related_documents": "festival_guide.pdf"  # OCR 처리됨
})

result = response.json()
print("생성된 안내문:", result['generation'])
print("날씨 요약:", result['weather_summary'])
```
...
```json
{
    "generation": "생성된 맞춤형 안전 안내문",
    "hallu_check": {
        "answer_with_citations": "Grounding 검증 완료된 안내문"
    },
    "weather_summary": "분석된 날씨 요약",
    "festival_query_list": ["행사 관련 검색어들"],
    "weather_query_list": ["날씨 관련 검색어들"],
    "safety_query_list": ["안전 관련 검색어들"]
}
```
...
1. **텍스트 추출**: PDF → `pdfplumber` 텍스트 추출
2. **OCR 처리**: 이미지 PDF → Google Cloud Vision API → 텍스트
3. **의미 기반 분할**: LangChain `SemanticChunker` + `gemini-embedding-001`
4. **컨텍스트 보강**: `gemini-2.5-flash-lite`로 문서 맥락 요약 → 헤더 생성
5. **벡터화**: 컨텍스트 보강 텍스트 → `gemini-embedding-001` → FAISS 저장

### 검색 파이프라인

1. **하이브리드 검색**:
   - 의미 검색(FAISS, 70%) + 키워드 검색(BM25, 30%)
   - 초기 결과: 150개 문서
...
#### chain_for_chat.py: 일반 채팅

```
질문 입력
    ↓
[Re-writer] 질문 재작성 (대명사 → 구체적 용어)
    ↓
[Question Decomposer] 복잡한 질문 → 하위 질문 분해
    ↓
[Search Document] 벡터 스토어 검색 → Re-ranking
    ↓
[Generator] 답변 생성 ('안전 지키미 AI' 역할)
    ↓
[Hallucination Checker] Grounding 확인 + 인용
    ↓
[Answer Beautifier] 마크다운 형식 정제
    ↓
최종 답변
```

#### chain_for_form.py: 양식 기반 안전 안내문

```
행사 정보 입력
    ↓
[Query Generator] 다각적 검색어 생성 (위험 시나리오 기반)
    ↓
[Search Document] 안전 관련 문서 검색
    ↓
[Generator] 맞춤형 안전 안내문 생성 ('안전 전문가' 역할)
    ↓
[Grounding & Beautify] 신뢰도 확인 + 마크다운 정제
    ↓
최종 안내문
```

#### custom.py: 날씨 기반 안전 분석

```
행사 정보 입력
    ↓
[Geocoding] 행사 장소명 → 위도/경도 변환
    ↓
[Weather API] Open-Meteo 24시간 날씨 예보 수집
    ↓
[날씨 분석] Gemini 2.5 Flash Lite로 위험 요소 추출
    ↓
다중 검색어 생성:
    - festival_query_generator (행사 관련)
    - weather_query_generator (날씨 관련)
    - safety_query_generator (일반 안전)
    ↓
다중 벡터 스토어 검색:
    - search_festival_document (행사 사고 사례)
    - search_weather_document (날씨 사고 사례)
    - search_safety_document (안전 문서)
    ↓
[Generator] 종합 안전 안내문 생성
    ↓
[Hallu Checker] Vertex AI Grounding 검증
    ↓
최종 결과: 안내문 + 날씨 요약 + 검색어 목록
```
...
| `GEOCODING_API` | Google Geocoding API 키 | 날씨 기반 기능 시 필수 |

---

## 라이선스

이 프로젝트의 라이선스 정보를 확인하세요.
